...
#
    ...
    file_output_stream = (suspicious_ip_df.select(F.to_json(F.struct("remote_host")).alias("value"))
      .writeStream                                   # Write the stream
      .format("kafka")
      .option("kafka.bootstrap.servers", args.host)
      .option("topic", "backlogs")
      .option("checkpointLocation", "file:///C:\Waqar\Docker\kafka_zookeeper_yaml\checkpoint")
      .outputMode("complete")
      .start())
